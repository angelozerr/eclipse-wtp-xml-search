	...
	public static void trace(byte level, String s, Throwable t) {
		...
		if (level == SEVERE) {
			if (!logged.contains(s)) {
				XMLJettyUIPlugin.getDefault().getLog().log(
						new Status(IStatus.ERROR, XMLJettyUIPlugin.PLUGIN_ID,
								s, t));
				logged.add(s);
			}
		}

		if (!XMLJettyUIPlugin.getDefault().isDebugging())
			return;

		StringBuilder sb = new StringBuilder(XMLJettyUIPlugin.PLUGIN_ID);
		sb.append(" ");
