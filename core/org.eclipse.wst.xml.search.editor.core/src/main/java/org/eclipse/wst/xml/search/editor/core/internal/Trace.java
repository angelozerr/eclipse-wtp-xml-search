	...
	public static void trace(byte level, String s, Throwable t) {
		...
		if (level == SEVERE) {
			if (!logged.contains(s)) {
				XMLSearchEditorCorePlugin.getDefault().getLog().log(
						new Status(IStatus.ERROR,
								XMLSearchEditorCorePlugin.PLUGIN_ID, s, t));
				logged.add(s);
			}
		}

		if (!XMLSearchEditorCorePlugin.getDefault().isDebugging())
			return;

		StringBuilder sb = new StringBuilder(XMLSearchEditorCorePlugin.PLUGIN_ID);
		sb.append(" ");
