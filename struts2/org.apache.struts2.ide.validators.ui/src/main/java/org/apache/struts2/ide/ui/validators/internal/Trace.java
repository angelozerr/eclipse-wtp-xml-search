	...
	public static void trace(byte level, String s, Throwable t) {
		...
		if (level == SEVERE) {
			if (!logged.contains(s)) {
				XWorkValidatorUIPlugin.getDefault().getLog().log(
						new Status(IStatus.ERROR, XWorkValidatorUIPlugin.PLUGIN_ID,
								s, t));
				logged.add(s);
			}
		}

		if (!XWorkValidatorUIPlugin.getDefault().isDebugging())
			return;

		StringBuilder sb = new StringBuilder(XWorkValidatorUIPlugin.PLUGIN_ID);
		sb.append(" ");
