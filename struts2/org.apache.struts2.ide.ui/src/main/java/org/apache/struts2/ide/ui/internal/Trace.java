	...
	public static void trace(byte level, String s, Throwable t) {
		...
		if (level == SEVERE) {
			if (!logged.contains(s)) {
				Struts2UIPlugin.getDefault().getLog().log(
						new Status(IStatus.ERROR, Struts2UIPlugin.PLUGIN_ID,
								s, t));
				logged.add(s);
			}
		}

		if (!Struts2UIPlugin.getDefault().isDebugging())
			return;

		StringBuilder sb = new StringBuilder(Struts2UIPlugin.PLUGIN_ID);
		sb.append(" ");
